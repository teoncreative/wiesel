...
namespace Wiesel {
  ...
  Reference<UniformBuffer> Renderer::CreateUniformBuffer(VkDeviceSize size) {
    Reference<UniformBuffer> uniformBuffer = CreateReference<UniformBuffer>();

    CreateBuffer(size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, uniformBuffer->m_Buffer, uniformBuffer->m_BufferMemory);

    WIESEL_CHECK_VKRESULT(vkMapMemory(m_LogicalDevice, uniformBuffer->m_BufferMemory, 0, size, 0, &uniformBuffer->m_Data));

    return uniformBuffer;
  }

  Reference<UniformBufferSet> Renderer::CreateUniformBufferSet(uint32_t frames, VkDeviceSize size) {
  ...
  Reference<Texture> Renderer::CreateBlankTexture() {
    Reference<Texture> texture = CreateReference<Texture>(TextureTypeDiffuse, "");
  ...
  Reference<Texture>
  Renderer::CreateTexture(const std::string& path, TextureProps textureProps, SamplerProps samplerProps) {
    Reference<Texture> texture = CreateReference<Texture>(textureProps.Type, path);
    ...
    CreateImage(texture->m_Width, texture->m_Height, texture->m_MipLevels, VK_SAMPLE_COUNT_1_BIT, textureProps.ImageFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, texture->m_Image, texture->m_DeviceMemory);

    TransitionImageLayout(texture->m_Image, textureProps.ImageFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, texture->m_MipLevels);
    CopyBufferToImage(stagingBuffer, texture->m_Image, static_cast<uint32_t>(texture->m_Width), static_cast<uint32_t>(texture->m_Height));
    ...
    // todo loading pregenerated mipmaps
    GenerateMipmaps(texture->m_Image, textureProps.ImageFormat, texture->m_Width, texture->m_Height, texture->m_MipLevels);
    texture->m_Sampler = CreateTextureSampler(texture->m_MipLevels, samplerProps);
    texture->m_ImageView = CreateImageView(texture->m_Image, textureProps.ImageFormat, VK_IMAGE_ASPECT_COLOR_BIT, texture->m_MipLevels);

    texture->m_IsAllocated = true;
    return texture;
  }

  Reference<DepthStencil> Renderer::CreateDepthStencil() {
    Reference<DepthStencil> texture = CreateReference<DepthStencil>();
  ...
  Reference<ColorImage> Renderer::CreateColorImage() {
    Reference<ColorImage> texture = CreateReference<ColorImage>();
  ...
  VkSampler Renderer::CreateTextureSampler(uint32_t mipLevels, SamplerProps samplerProps) {
    ...
    if (samplerProps.MaxAnisotropy <= 0) {
      ...
      samplerInfo.maxAnisotropy = std::min(samplerProps.MaxAnisotropy, properties.limits.maxSamplerAnisotropy);
    }
    ...
    VkSampler sampler;
    WIESEL_CHECK_VKRESULT(vkCreateSampler(m_LogicalDevice, &samplerInfo, nullptr, &sampler));
    return sampler;
  }
  ...
  Renderer::CreateDescriptors(Reference<UniformBufferSet> uniformBufferSet, Reference<Material> material) {
    Reference<DescriptorData> object = CreateReference<DescriptorData>(k_MaxFramesInFlight);

    VkDescriptorPoolSize poolSizes[] = {
        {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, k_MaxFramesInFlight * 2},
        {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, k_MaxFramesInFlight * k_MaterialTextureCount}};
    ...
    WIESEL_CHECK_VKRESULT(vkAllocateDescriptorSets(m_LogicalDevice, &allocInfo, object->m_DescriptorSets.data()));
  ...
  void
  Renderer::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
  ...
  void
  Renderer::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) {
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
  ...
  void Renderer::CreateCommandBuffers() {
    ...
    WIESEL_CHECK_VKRESULT(vkAllocateCommandBuffers(m_LogicalDevice, &allocInfo, m_CommandBuffers.data()));
  }
  ...
  void
  Renderer::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
  ...
  void
  Renderer::GenerateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
  ...
  void Renderer::CreateGlobalUniformBuffers() {
    m_LightsGlobalUboSet = CreateUniformBufferSet(k_MaxFramesInFlight, sizeof(LightsUniformBufferObject) + 16);
  }
  ...
  bool Renderer::BeginFrame(Reference<CameraData> data) {
    ...
    WIESEL_CHECK_VKRESULT(vkBeginCommandBuffer(m_CommandBuffers[m_CurrentFrame], &beginInfo));
