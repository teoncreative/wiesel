...
namespace Wiesel {
  ...
  struct ExposedVariable {
    ExposedVariable(T value, const std::string& name) : Value(value),
                                                        Name(name) {}
  ...
  class IBehavior {
  public:
    IBehavior(const std::string& name, Entity entity, const std::string& file) : m_Name(name),
                                                                                 m_Entity(entity),
                                                                                 m_File(file),
                                                                                 m_InternalBehavior(false),
                                                                                 m_Enabled(true),
                                                                                 m_Unset(false) {}
    IBehavior(const std::string& name, Entity entity) : m_Name(name),
                                                        m_Entity(entity),
                                                        m_File("Internal"),
