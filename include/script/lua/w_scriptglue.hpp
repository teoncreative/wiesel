...
namespace Wiesel::ScriptGlue {
  class ScriptVec3 {
  public:
    ScriptVec3(glm::vec3& backingVec3, bool* changedPtr) : m_BackingVec3(backingVec3),
                                                           m_ChangedPtr(changedPtr) {}
    ...
    void SetX(float v) {
      m_BackingVec3.x = v;
      if (m_ChangedPtr) *m_ChangedPtr = true;
    }
    void SetY(float v) {
      m_BackingVec3.y = v;
      if (m_ChangedPtr) *m_ChangedPtr = true;
  ...
  struct ScriptTransformComponent {
    ScriptTransformComponent(TransformComponent& transform) : Transform(transform),
                                                              Position({transform.Position, &transform.IsChanged}),
                                                              Rotation({transform.Rotation, &transform.IsChanged}),
                                                              Scale({transform.Scale, &transform.IsChanged}) {}
