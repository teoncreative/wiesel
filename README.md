...
- **Deferred PBR rendering** with render graph, cascaded shadow maps, ray-traced shadows, SSAO, bloom, motion blur,
  TAA/FXAA, IBL, and forward transparency
- **Jolt Physics** with multi-threaded simulation, box/sphere/capsule/mesh/heightfield colliders, triggers, one-way
  platforms, raycasting, and overlap queries
...
- **UI system** with canvas layout, anchors/pivots, text rendering, buttons, text input, and gamepad navigation
- **Input system** with data-driven contexts, multi-player support, and gamepad mapping
- **Asset system** with VFS, stable handles via .meta files, async loading, and .pak archive support
