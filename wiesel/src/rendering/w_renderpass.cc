...
RenderPass::RenderPass(PassType pass_type, const std::string& debug_name)
...
void RenderPass::AttachOutput(std::shared_ptr<AttachmentTexture> attachment) {
  attachments_.push_back({.type = attachment->type_,
...
void RenderPass::Bake() {
  ...
  for (const auto& item : attachments_) {
    if (item.type == AttachmentTextureType::DepthStencil &&
        depthAttachmentRefs.empty()) {
      bool loads_depth = pass_type_ == PassType::Lighting ||
                         pass_type_ == PassType::ForwardTransparency;
      descriptions.push_back(
          {.format = item.format,
           ...
           .initialLayout =
               loads_depth ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL
      ...
      index++;
    } else if (item.type == AttachmentTextureType::Color ||
      ...
      index++;
    } else if (item.type == AttachmentTextureType::Resolve ||
      ...
      if (used_as_resolve) {
        descriptions.push_back({
            ...
            .finalLayout = pass_type_ == PassType::Present
...
void RenderPass::Begin(std::shared_ptr<Framebuffer> framebuffer,
  ...
  for (const auto& item : attachments_) {
    if (item.type == AttachmentTextureType::DepthStencil) {
      ...
      clearValues.push_back({.color = {clear_color.red, clear_color.green,
...
std::shared_ptr<Framebuffer> RenderPass::CreateFramebuffer(
  ...
  for (const auto& item : output_attachments) {
    if (item->type_ == AttachmentTextureType::DepthStencil && !has_depth) {
      ...
      has_depth = true;
    } else if (item->type_ == AttachmentTextureType::Color ||
               item->type_ == AttachmentTextureType::Offscreen ||
               item->type_ == AttachmentTextureType::SwapChain ||
