...
std::shared_ptr<Texture> GetOrCreateLabelTexture(
  ...
  std::vector<uint8_t> font_data(font_file.Data(), font_file.Data() + font_file.Size());
  ...
  for (char c : text) {
    ...
    for (unsigned int row = 0; row < bmp.rows; row++) {
      for (unsigned int col = 0; col < bmp.width; col++) {
        ...
        pixels[idx + 0] = static_cast<uint8_t>(txt_r * a + pixels[idx + 0] * (1.0f - a));
        pixels[idx + 1] = static_cast<uint8_t>(txt_g * a + pixels[idx + 1] * (1.0f - a));
        pixels[idx + 2] = static_cast<uint8_t>(txt_b * a + pixels[idx + 2] * (1.0f - a));
        pixels[idx + 3] = static_cast<uint8_t>(txt_a * a + pixels[idx + 3] * (1.0f - a));
      }
    }
